    ...
    public class WebCrawler {
        ...
        public async Task<URL> Crawl(string url) {
            try {
                ...
                // there is really no good way to look up to see if something has been crawled already... sorta...
                // if it's been seen before it will have the domain and hash with no title like usual. when it is 
                // crawled, it should have the words broken out and there will always be the title-less entry here.
                ...
                Debug.WriteLine("[-] Something terrible has happened... :/ " + e.ToString());
                await store.reportToWebRole("[-] Error in Crawl; something terrible has happened... " + e.ToString());
                return null;
            }
        }
        ...
        public static bool validDate(string date) {
            try {
                ...
                store.reportToWebRole("[-] Error in parsing date from url: " + date);
                return true;
            }
        ...
        public static string[] parseURL(string url) {
            ...
            string[] result;
            if (WorkerRole.urlComponentsCache.TryGetValue(url, out result)) {
                return result;
            } else {
                result = new string[5];
            }
            bool valid = false;
            Match match = Regex.Match(url, @"^(?:(https?):)?\/\/(.*?)(?:(\/.*?))?(?:\?(.*?))?(?:#(.*))?$", RegexOptions.Singleline | RegexOptions.IgnoreCase);
            if (match.Success) {
                valid = true;
            } else if ((match = Regex.Match(url, @"^()()(?:(\/(?:[^/].*?)?))(?:\?(.*?))?(?:#(.*))?$", RegexOptions.IgnoreCase | RegexOptions.Singleline)).Success) {
                valid = true;
            } else {
                if (WorkerRole.DEBUG) {
            ...
            if (valid) {
                for (var i = 0; i < result.Length; i++) {
                    try {
                        ...
                        store.reportToWebRole("[-] Regex Matching Error: " + url);
                        result[i] = "";
                    }
                }
            }
        ...
        public static string parseDomain(string[] urlComponents) {
            try {
                ...
                Debug.WriteLine("[-] Error in parseDomain: " + e.ToString() + " => "
                    + urlComponents[0] + "://" + urlComponents[1] + urlComponents[2]
                    + "?" + urlComponents[3] + "#" + urlComponents[4]);
                store.reportToWebRole("[-] Error in parseDomain: " + e.ToString() + " => "
                    + urlComponents[0] + "://" + urlComponents[1] + urlComponents[2]
                    + "?" + urlComponents[3] + "#" + urlComponents[4]);
                return null;
            }
        ...
        private async Task<URL> parseHTML(string url, string htmlString) {
            Match match = Regex.Match(htmlString, @"<title.*?>(.*?)<\/title>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            string title = null;
            if (match.Success) {
                title = match.Groups[1].Value;
            } else {
                Debug.WriteLine("[-] This page has no title: " + url);
                await store.reportToWebRole("[-] Page missing a title: " + url);
            }
            MatchCollection links = Regex.Matches(htmlString, "<a .*?href=\"?(.*?)(?:\"| ).*? ?>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            if (links.Count > 0) {
                if (WorkerRole.DEBUG) {
                    Debug.WriteLine("[*] Pulled " + links.Count + " links off of page");
                }
                Task.Run(() => getLinks(links, url)); // I think i want to specifically not await here...
            } else {
                Debug.WriteLine("[-] No links to grab on " + url);
                await store.reportToWebRole("[-] No links to grab on " + url);
            }
            // loop through title for words
            Scanner scan = new Scanner(title.ToLower());
            URL result = null;
            while (scan.hasNext()) {
                ...
                if (word.Length > 0) {
                    ...
                    result = await store.addToURLTable(url, title, URL.sha256(word));
                }
            }
            return result;
        }
        ...
        private async Task getLinks(MatchCollection links, string url) {
            try {
                        ...
                        if (await store.getURLData(earl) == null) { // we don't have data on it
                            if (await store.AddToQueue(earl, false)) {
                                ...
                                try {
                                    ...
                                    tmp[tmp.Count - 1].Add(new URL(earl));
                                } catch (Exception e) {
                                    Debug.WriteLine("[-] Batch Uniqueness Preparation Error: " + e.ToString());
                                    await store.reportToWebRole("[-] Batch Uniqueness Preparation Error: " + e.ToString());
                                }
                ...
                Debug.WriteLine("[-] Error in getLinks: " + e.ToString());
                await store.reportToWebRole("[-] Error in getLinks: " + e.ToString());
                return;
            }
        }
        ...
        public async Task<WebResponse> httpRequest(string url) {
            ...
            try {
                ...
                await store.reportToWebRole("[-] Error in httpRequest: " + e.Message + " when attempting to access " + url);
                WorkerRole.errors.Enqueue(new KeyValuePair<string, Exception>(url, e));
                await store.addToURLTable(url, e.Message, null); // add to url table with the error as the title
                return null;
            }
        }
